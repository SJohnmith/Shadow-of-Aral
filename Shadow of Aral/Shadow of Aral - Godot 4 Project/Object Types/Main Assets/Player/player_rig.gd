...
var max_recoil: float = 20.0        # Max Recoil
...
func player_wpn_action(action):
     ...
     # Check that the Weapon Has Method Shoot
          if path_to_wpn.has_method("shoot"):
               ...
               var recoil_rad_actual = deg_to_rad(randf_range(-recoil_deg_max, recoil_deg_max))
               $Torso/Weapon.rotate(recoil_rad_actual)
...
func wpn_arm_recoil():
     ...
     var random_recoil_rotation = randf_range(deg_to_rad(max_recoil), deg_to_rad(25))
     tween.tween_property(front_arm, "rotation", 0.1 , 0.1)
